...
// Multer in-memory storage (no disk usage)
const upload = multer({
...
// Generate filename
const generateFileName = (folder, original) => {
...
/*---------------------------------------------------------------------
  SINGLE IMAGE UPLOAD
---------------------------------------------------------------------*/
const singleImageUpload = (field = "image", folder = "products") => {
    return async (req, res, next) => {
        ...
        handler(req, res, async (err) => {
            ...
            try {
                const filename = generateFileName(folder, req.file.originalname);

                // Upload to Vercel Blob
                const blob = await put(`uploads/${folder}/${filename}`, req.file.buffer, {
                    access: "public"
                });
...
---------------------------------------------------------------------*/
const multipleImageUpload = (field = "images", folder = "products", maxCount = 10) => {
    return async (req, res, next) => {
        ...
        handler(req, res, async (err) => {
            ...
            try {
                ...
                for (const file of req.files) {
                    ...
                    const blob = await put(`uploads/${folder}/${filename}`, file.buffer, {
                        access: "public"
                    });
    ...
    return async (req, res, next) => {
        handler(req, res, async (err) => {
            ...
            try {
                ...
                if (req.files?.image?.[0]) {
                    const file = req.files.image[0];
                    const filename = generateFileName("categories", file.originalname);

                    const blob = await put(`uploads/categories/${filename}`, file.buffer, {
                        access: "public"
                    });
